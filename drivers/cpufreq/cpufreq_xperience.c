...
static DEFINE_PER_CPU(struct xperience_info_s, xperience_info);
...
enum {
	XPERIENCE_DEBUG_JUMPS=1,
	XPERIENCE_DEBUG_LOAD=2,
	XPERIENCE_DEBUG_ALG=4
};
...
static int cpufreq_governor_xperience(struct cpufreq_policy *policy,
		unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_XPERIENCE
static
#endif
struct cpufreq_governor cpufreq_gov_xperience = {
	.name = "xperience",
	.governor = cpufreq_governor_xperience,
	.max_transition_latency = 9000000,
	.owner = THIS_MODULE,
};

inline static void xperience_update_min_max(struct xperience_info_s *this_xperience, struct cpufreq_policy *policy, int suspend) {
	if (suspend) {
		this_xperience->ideal_speed = // sleep_ideal_freq; but make sure it obeys the policy min/max
			policy->max > sleep_ideal_freq ?
			(sleep_ideal_freq > policy->min ? sleep_ideal_freq : policy->min) : policy->max;
	} else {
		this_xperience->ideal_speed = // awake_ideal_freq; but make sure it obeys the policy min/max
			policy->min < awake_ideal_freq ?
			(awake_ideal_freq < policy->max ? awake_ideal_freq : policy->max) : policy->min;
	}
}

inline static void xperience_update_min_max_allcpus(void) {
	unsigned int i;
	for_each_online_cpu(i) {
		struct xperience_info_s *this_xperience = &per_cpu(xperience_info, i);
		if (this_xperience->enable)
			xperience_update_min_max(this_xperience,this_xperience->cur_policy,suspended);
	}
}
...
inline static void reset_timer(unsigned long cpu, struct xperience_info_s *this_xperience) {
	this_xperience->time_in_idle = get_cpu_idle_time_us(cpu, &this_xperience->idle_exit_time);
	mod_timer(&this_xperience->timer, jiffies + sample_rate_jiffies);
}
...
inline static int target_freq(struct cpufreq_policy *policy, struct xperience_info_s *this_xperience,
			      int new_freq, int old_freq, int prefered_relation) {
	int index, target;
	struct cpufreq_frequency_table *table = this_xperience->freq_table;
	    ...
	    !cpufreq_frequency_table_target(policy,table,new_freq,prefered_relation,&index))
	{
		...
		if (target == old_freq) {
			...
			printk(KERN_WARNING "xperience: frequency change failed: %d to %d => %d\n",
	...
	dprintk(XPERIENCE_DEBUG_JUMPS,"xperienceQ: jumping from %d to %d => %d (%d)\n",
		old_freq,new_freq,target,policy->cur);

	return target;
}

static void cpufreq_xperience_timer(unsigned long cpu)
{
	u64 delta_idle;
	u64 delta_time;
	int cpu_load;
	...
	u64 now_idle;
	int queued_work = 0;
	struct xperience_info_s *this_xperience = &per_cpu(xperience_info, cpu);
	struct cpufreq_policy *policy = this_xperience->cur_policy;

	now_idle = get_cpu_idle_time_us(cpu, &update_time);
	old_freq = policy->cur;

	if (this_xperience->idle_exit_time == 0 || update_time == this_xperience->idle_exit_time)
		return;
	...
	dprintk(XPERIENCE_DEBUG_LOAD,"xperienceT @ %d: load %d (delta_time %llu)\n",
		old_freq,cpu_load,delta_time);

	this_xperience->cur_cpu_load = cpu_load;
	this_xperience->old_freq = old_freq;
	...
	if (cpu_load > max_cpu_load || delta_idle == 0)
	{
		if (old_freq < policy->max &&
			 (old_freq < this_xperience->ideal_speed || delta_idle == 0 ||
			  cputime64_sub(update_time, this_xperience->freq_change_time) >= up_rate_us))
		{
			dprintk(XPERIENCE_DEBUG_ALG,"xperienceT @ %d ramp up: load %d (delta_idle %llu)\n",
				old_freq,cpu_load,delta_idle);
			this_xperience->ramp_dir = 1;
			work_cpumask_set(cpu);
			queue_work(up_wq, &freq_scale_work);
			queued_work = 1;
		}
		else this_xperience->ramp_dir = 0;
	}
	...
	// frequency we require that we have been at this frequency for at least down_rate_us:
		 ...
		 (old_freq > this_xperience->ideal_speed ||
		  cputime64_sub(update_time, this_xperience->freq_change_time) >= down_rate_us))
	{
		dprintk(XPERIENCE_DEBUG_ALG,"xperienceT @ %d ramp down: load %d (delta_idle %llu)\n",
			old_freq,cpu_load,delta_idle);
		this_xperience->ramp_dir = -1;
		work_cpumask_set(cpu);
...
{
	struct xperience_info_s *this_xperience = &per_cpu(xperience_info, smp_processor_id());
	struct cpufreq_policy *policy = this_xperience->cur_policy;
	...
	if (policy->cur == policy->min && timer_pending(&this_xperience->timer))
		del_timer(&this_xperience->timer);
...
static int cpufreq_idle_notifier(struct notifier_block *nb,
	...
	if (val == IDLE_START) {
		if (policy->cur == policy->max && !timer_pending(&this_xperience->timer)) {
			reset_timer(smp_processor_id(), this_xperience);
...
static void cpufreq_xperience_freq_change_time_work(struct work_struct *work)
{
	unsigned int cpu;
	int new_freq;
	int old_freq;
	...
	for_each_possible_cpu(cpu) {
		...
		ramp_dir = this_xperience->ramp_dir;
		this_xperience->ramp_dir = 0;

		old_freq = this_xperience->old_freq;
		policy = this_xperience->cur_policy;

		if (old_freq != policy->cur) {
			// frequency was changed by someone else?
			printk(KERN_WARNING "xperience: frequency changed by 3rd party: %d to %d\n",
			       old_freq,policy->cur);
			new_freq = old_freq;
		}
		else if (ramp_dir > 0 && nr_running() > 1) {
			// ramp up logic:
			if (old_freq < this_xperience->ideal_speed)
				new_freq = this_xperience->ideal_speed;
			...
			dprintk(XPERIENCE_DEBUG_ALG,"xperienceQ @ %d ramp up: ramp_dir=%d ideal=%d\n",
				old_freq,ramp_dir,this_xperience->ideal_speed);
		}
		else if (ramp_dir < 0) {
			// ramp down logic:
			if (old_freq > this_xperience->ideal_speed) {
				...
				new_freq = old_freq * this_xperience->cur_cpu_load / max_cpu_load;
				if (new_freq > old_freq) // min_cpu_load > max_cpu_load ?!
					new_freq = old_freq -1;
			}
			dprintk(XPERIENCE_DEBUG_ALG,"xperienceQ @ %d ramp down: ramp_dir=%d ideal=%d\n",
				old_freq,ramp_dir,this_xperience->ideal_speed);
		}
			...
			new_freq = old_freq;
			dprintk(XPERIENCE_DEBUG_ALG,"xperienceQ @ %d nothing: ramp_dir=%d nr_running=%lu\n",
				old_freq,ramp_dir,nr_running());
		}

		// do actual ramp up (returns 0, if frequency change failed):
		new_freq = target_freq(policy,this_xperience,new_freq,old_freq,relation);
		if (new_freq)
			this_xperience->freq_change_time_in_idle =
				get_cpu_idle_time_us(cpu,&this_xperience->freq_change_time);
		...
		else if (timer_pending(&this_xperience->timer))
			del_timer(&this_xperience->timer);

		cpufreq_notify_utilization(policy,
			(this_xperience->cur_cpu_load * policy->cur) / policy->max);
	}
}
...
static struct attribute * xperience_attributes[] = {
...
static struct attribute_group xperience_attr_group = {
	.attrs = xperience_attributes,
	.name = "xperience",
};

static int cpufreq_governor_xperience(struct cpufreq_policy *new_policy,
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_xperience->cur_policy = new_policy;

		this_xperience->enable = 1;

		xperience_update_min_max(this_xperience,new_policy,suspended);

		this_xperience->freq_table = cpufreq_frequency_get_table(cpu);
		if (!this_xperience->freq_table)
			printk(KERN_WARNING "xperience: no frequency table for cpu %d?!\n",cpu);
	...
	case CPUFREQ_GOV_LIMITS:
		...
		if (this_xperience->cur_policy->cur > new_policy->max) {
			dprintk(XPERIENCE_DEBUG_JUMPS,"xperienceI: jumping to new max freq: %d\n",new_policy->max);
	...
	case CPUFREQ_GOV_STOP:
		this_xperience->enable = 0;
		smp_wmb();
		del_timer(&this_xperience->timer);
		flush_work(&freq_scale_work);
		this_xperience->idle_exit_time = 0;
...
static void xperience_suspend(int cpu, int suspend)
{
	struct xperience_info_s *this_xperience = &per_cpu(xperience_info, smp_processor_id());
	struct cpufreq_policy *policy = this_xperience->cur_policy;
	...
	xperience_update_min_max(this_xperience,policy,suspend);
	if (!suspend) { // resume at max speed:
		...
		dprintk(XPERIENCE_DEBUG_JUMPS,"xperienceS: suspending at %d\n",policy->cur);
	}
...
static int __init cpufreq_xperience_init(void)
{
	unsigned int i;
	struct xperience_info_s *this_xperience;
	debug_mask = 0;
	...
	for_each_possible_cpu(i) {
		this_xperience = &per_cpu(xperience_info, i);
		this_xperience->enable = 0;
		this_xperience->cur_policy = 0;
		this_xperience->ramp_dir = 0;
		this_xperience->time_in_idle = 0;
		this_xperience->idle_exit_time = 0;
		this_xperience->freq_change_time = 0;
		this_xperience->freq_change_time_in_idle = 0;
		this_xperience->cur_cpu_load = 0;
		// intialize timer:
		init_timer_deferrable(&this_xperience->timer);
		this_xperience->timer.function = cpufreq_xperience_timer;
		this_xperience->timer.data = i;
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_XPERIENCE
fs_initcall(cpufreq_xperience_init);
#else
module_init(cpufreq_xperience_init);
#endif

static void __exit cpufreq_xperience_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_xperience);
	destroy_workqueue(up_wq);
	destroy_workqueue(down_wq);
}

module_exit(cpufreq_xperience_exit);
