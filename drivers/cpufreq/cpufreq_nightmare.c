 ...
 * 
 *  Copyright (C)  2014 Carlos "Klozz" Jesús B.C aka TeamMEX <xxx.reptar.rawrr.xxx@gmail.com>
 *
...
static void nightmare_check_cpu(struct cpufreq_nightmare_cpuinfo *this_nightmare_cpuinfo)
{
	...
	for_each_possible_cpu(j) {
		...
		cur_idle_time = kcpustat_cpu(j).cpustats.idle;
		cur_wall_time = cputime64_add(kcpustat_cpu(j).cpustats.user,
			kcpustat_cpu(j).cpustats.system);
		cur_wall_time = cputime64_add(cur_wall_time,
			kcpustat_cpu(j).cpustats.iowait);
		cur_wall_time = cputime64_add(cur_wall_time,
			kcpustat_cpu(j).cpustats.irq);
		cur_wall_time = cputime64_add(cur_wall_time,
			kcpustat_cpu(j).cpustats.softirq);
		cur_wall_time = cputime64_add(cur_wall_time,
			kcpustat_cpu(j).cpustats.steal);
		cur_wall_time = cputime64_add(cur_wall_time,
			kcpustat_cpu(j).cpustats.nice);
		cur_wall_time = cputime64_add(cur_wall_time,
			cur_idle_time);
				...
				unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		j_nightmare_cpuinfo->prev_cpu_idle = kcpustat_cpu(j).cpustats.idle;
		j_nightmare_cpuinfo->prev_cpu_wall = cputime64_add(kcpustat_cpu(j).cpustats.user,
			kcpustat_cpu(j).cpustats.system);
